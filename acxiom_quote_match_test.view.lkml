...
FROM ((SELECT *,
              '0' AS new
       FROM v_acxiom_quote_match)
       UNION ALL
       (SELECT *,
              '1' AS new
       FROM v_acxiom_test_quote_match)) a
  ...
  measure: amount_new {
    type:  number
    sql: 1.0*SUM(CASE WHEN ${TABLE}.new = 1 THEN 1 ELSE 0 END)/ 919010496
      ;;
  }
  ...
  measure: amount_old {
    type:  number
    sql: 1.0*SUM(CASE WHEN ${TABLE}.new = 1 THEN 0 ELSE 1 END)/ 1894756774
      ;;
  }
